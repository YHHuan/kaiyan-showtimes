...
export function versionSignature(title) {
  ...
  return [...new Set(clean.match(/導演(?:剪輯)?版|導演剪輯|加長版|完整版|特別版|未刪減版|院線版|原版|國語|日語|英語|台語|粵語|韓語|配音|國配|真人版|字幕|\b(?:19|20)\d{2}\b|\bdirector['’]?s?\s*cut\b|\b(?:extended|uncut|theatrical)\s*(?:edition|version|cut)?\b/g) || [])].sort().join('|');
}
...
const VERSION_MARK_RE = /國語|日語|英語|台語|粵語|韓語|配音|國配|真人版|字幕|導演|加長|完整|特別版|未刪減|院線版|原版|(?:19|20)\d{2}|director|extended|uncut|theatrical|\d\s?D$/i;
...
export function trustedMovieMeta(title, record) {
  return record?.matchVersion === 2 && typeof record.matchedTitle === 'string'
