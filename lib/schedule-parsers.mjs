...
// 舊版曾把頁尾 li 併入最後一部電影。只清除已確認的開眼版權文字，
// 不用長度上限／格式白名單刪掉合法的語言、影廳或活動標籤。
// 同一規則也用於前端舊收藏的 tag；tests/labels.mjs 驗證兩邊相容。
export function cleanAtmoviesTags(tags = []) {
  return [...new Set((Array.isArray(tags) ? tags : []).filter(t => typeof t === 'string').map(t => t
    .replace(/(?:^|・)\s*(?:&copy;|&#169;|&#x0*a9;|©)?\s*@movies\s+All rights reserved\s+開眼電影網版權所有\s*$/i, '')
    .trim()).filter(Boolean))];
}

function atmoviesTables(html) {
  const tables = [];
  let start = -1, depth = 0;
  // 場次表內還有巢狀 ul，不能停在第一個 </ul>，也不能依賴頁面註解的拼字。
  for (const token of html.matchAll(/<\/?ul\b[^>]*>/gi)) {
    const close = /^<\//.test(token[0]);
    const table = !close && /\sid\s*=\s*(?:"theaterShowtimeTable"|'theaterShowtimeTable'|theaterShowtimeTable(?=\s|>))/i.test(token[0]);
    if (table) {
      if (start >= 0) throw new Error('場次解析不完整：上一個場次清單未關閉');
      start = token.index;
      depth = 0;
    }
    if (start < 0) continue;
...
export function parseAtmovies(html, { code, region, name, area, official, expectedDate = todayISO() }) {
  ...
  const blocks = atmoviesTables(html);
  let broken = 0;
  for (const block of blocks) {
    ...
    const tags = cleanAtmoviesTags([...block.matchAll(/<li(?:\s[^>]*)?>\s*([^<]+?)\s*<\/li>/gi)]
