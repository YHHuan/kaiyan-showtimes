...
export function parseMiranew(html) {
        ...
        for (const hallGroup of day.ShowTimeList || []) {
          for (const s of hallGroup.SessionList || []) {
            records.push({
              ...
              sourceRuntimeMin: Number(mv.MovieLength) || null,
              date,
              time: s.ShowTime,
  ...
  if (!records.length) throw new Error('美麗新官網未解析到場次，保留上一輪資料');
  return records;
}

if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  const html = await politeFetch('https://www.miranewcinemas.com/booking/timetable');
  await saveRecords(new URL('../data/miranew.json', import.meta.url).pathname, parseMiranew(html));
}
