...
// 整支抓取器拋錯也要記錄本輪失敗；不能讓 12 小時前的「成功」壓過剛抓到的備援。
// 保留來源／各筆原本的 fetchedAt，這不是一次成功更新。
export async function markSourceFailed(path) {
  const source = path.replace(/.*\//, '').replace(/\.json$/, '');
  const statusPath = `${dirname(path)}/_status.json`;
  let status;
  try { status = JSON.parse(await readFile(statusPath, 'utf8')); } catch { return; }
  const old = status[source];
  if (!old) return; // 票價、metadata 等非場次任務沒有此狀態紀錄。
  let rows = [];
  try { rows = JSON.parse(await readFile(path, 'utf8')); } catch {}
  if (!Array.isArray(rows)) return;
  const stamp = new Date().toISOString();
  const cinemas = Object.fromEntries(Object.entries(old.cinemas || {}).map(([name, value]) => [cinemaName(name), value]));
  for (const r of rows) {
    const name = cinemaName(r.cinema);
    if (!cinemas[name]) cinemas[name] = { area: areaName(r.area), url: r.url, dates: [], lastSuccessAt: old.fetchedAt };
    if (!cinemas[name].dates?.includes(r.date)) cinemas[name].dates = [...(cinemas[name].dates || []), r.date];
  }
  for (const c of Object.values(cinemas)) {
    c.state = 'failed'; c.attemptedAt = stamp;
    c.failedDates = [...new Set([...(c.failedDates || []), ...(c.dates || []), todayISO()])].filter(d => d >= todayISO()).sort();
  }
  status[source] = { ...old, lastAttemptAt: stamp, lastAttemptState: 'failed', cinemas };
...
export function versionSignature(title) {
  ...
  return [...new Set(clean.match(/導演(?:剪輯)?版|導演剪輯|加長版|加碼|完整版|特別版|未刪減版|院線版|原版|國語|日語|英語|台語|粵語|韓語|配音|國配|真人版|動畫(?:版)?|字幕|\b(?:19|20)\d{2}\b|\bdirector['’]?s?\s*cut\b|\b(?:extended|uncut|theatrical)\s*(?:edition|version|cut)?\b/g) || [])].sort().join('|');
}
...
const VERSION_MARK_RE = /國語|日語|英語|台語|粵語|韓語|配音|國配|真人版|動畫|字幕|導演|加長|加碼|完整|特別版|未刪減|院線版|原版|(?:19|20)\d{2}|director|extended|uncut|theatrical|\d\s?D$/i;
